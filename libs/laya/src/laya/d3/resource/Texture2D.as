package laya.d3.resource {
	...
	public class Texture2D extends BaseTexture {
		...
		override public function onAsynLoaded(url:String, data:*, params:Array):void {
			if (params) {
				var canRead:Boolean = params[0];
				(canRead) && (_canRead = canRead);
				var repeat:Boolean = params[1];
				(repeat) && (_repeat = repeat);
				var format:int = params[2];
				(format) && (_format = format);
				var mipmap:Boolean = params[3];
				(mipmap) && (_mipmap = mipmap);
			}
